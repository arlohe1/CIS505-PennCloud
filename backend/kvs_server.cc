...
int put(char* buf, int len, int comm_fd) {
	char* row = strtok(buf, ",");
...
static void* tExecute(void *arg) {
	...
	while((bytesToRead > 0) && (result = read(comm_fd, ptr, bytesToRead)) > 0) {
	
		debugDetailed("number bytes read: %d\n", result);
		ptr = ptr + result;
		bytesToRead = bytesToRead - result;
		totalRead = totalRead + result;
		while ((indx = containsCrlf(buffer, totalRead)) != -1) {
...
int main(int argc, char *argv[]) {	
	...
	int port = 10000;
	debugFlag = 0;
	const char* greeting = "+OK KV Server ready\r\n";
	///int socketfd = -1;
