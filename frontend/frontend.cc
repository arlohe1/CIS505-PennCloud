...
std::string putKVS(std::string row, std::string column, std::string value) {
       ...
       std::string request = "PUT " + std::to_string(cmdLength) + "," + row + "," + column + "," + value;
       log("KVS PUT: " + request);
       ...
       if (req.filepath.compare("/") == 0) {
              if (req.cookies.find("username") == req.cookies.end()) {
                     ...
                     if (req.cookies.find("error") != req.cookies.end()) {
                            test = "<p style=\"color:red\";>" + req.cookies["error"] + "</p><br/>";
                     ...
                     resp.content =
                         ...
                         "center;\">" +
                         test + "<form id=\"login\" style=\"display:" + (signuperr ? "none" : "block") +
              ...
              if (req.cookies.find("username") == req.cookies.end()) {
                     if (req.formData["username"] == "" || getKVS(req.formData["username"], "password") == "") {
                            resp.status_code = 307;
                            resp.status = "Temporary Redirect";
                            resp.headers["Location"] = "/";
                            resp.cookies["error"] = "Invalid username.";
                     } else if (getKVS(req.formData["username"], "password") != req.formData["password"]) {
                            resp.status_code = 307;
                            resp.status = "Temporary Redirect";
                            resp.headers["Location"] = "/";
              ...
              if (req.cookies.find("username") == req.cookies.end()) {
                     ...
                     if (req.formData["username"].size() == 0 || !valid) {
                            ...
                            resp.cookies["signuperr"] = "1";
                                ...
                                std::all_of(req.formData["password"].begin(), req.formData["password"].end(),
                                            isspace)) {
                            resp.status_code = 307;
                            resp.status = "Temporary Redirect";
                            resp.headers["Location"] = "/";
                            resp.cookies["error"] = "Password cannot be empty or only spaces.";
                            resp.cookies["signuperr"] = "1";
                     } else if (req.formData["password"] != req.formData["confirm_password"]) {
                            resp.status_code = 307;
                            resp.status = "Temporary Redirect";
                            resp.headers["Location"] = "/";
                            ...
                            putKVS(req.formData["username"], "password", req.formData["password"]);
                            resp.status_code = 307;
                            resp.status = "Temporary Redirect";
                            resp.headers["Location"] = "/dashboard";
                            resp.cookies["username"] = req.formData["username"];
