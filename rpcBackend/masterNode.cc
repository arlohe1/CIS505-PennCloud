...
std::map<int, std::list<std::string>> clusterToActiveNodesMap;
...
std::tuple<int, std::string> getNewClusterLeader(std::string oldLeader) {
    ...
    std::list<std::string> serverList = clusterToActiveNodesMap[currCluster];
    serverList.remove(oldLeader);
    ...
    while(fgets(buffer, 300, f)){
        ...
        if(serverNum != -1) {
            ...
            if(clusterToActiveNodesMap.count(currCluster) <= 0) {
                clusterToActiveNodesMap[currCluster] = std::list<std::string> {};
            }
        } else {
