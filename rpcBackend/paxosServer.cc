   ...
   // if (val.length() + cacheSize <= maxCache) {
		...
		if (kvMap.count(row) > 0) {
			if (kvMap[row].count(col) > 0) {
				newEntry = kvMap[row][col];
			}
		}
    	kvMap[row][col] = newEntry + val + "\n";
    	kvLoc[row][col] = 1;
...
// int roundNum;
// int proposalNum; // set to round num*100 + server num
// int seqNum; // set seqNum
// int nextSeqNum = 1;
using prepare_tuple = std::tuple<int, int, std::string>;  // status code, acceptedSeqNum, acceptedVal
...
resp_tuple learn(int seqNum, std::string row, std::string col, std::string val) {

	std::map<std::string, prepare_tuple> respSet; // map from ip:port -> resp tuple
	prepare_tuple resp;

	// decie for self and for other nodes
	decide(seqNum, row, col, val);
	        ...
	        if(otherServer.compare(myAddrPortForFrontend) != 0 && clusterNodesToSkip.count(otherServer) <= 0) {
	            ...
	            while(continueTrying) {
	                ...
	                try { // on timeout, query connection state and retry if connected
	                    ...
	                    client.call("decide", seqNum, row, col, val);
	                    continueTrying = false;
	                } catch (rpc::timeout &t) {
	                    debugDetailed("%s for (%s, %s) with server %s timed out!\n", "decide", row.c_str(), col.c_str(), otherServer.c_str());
	                    // connect to heartbeat thread of backend server and check if it's alive w/ shorter timeout
...
int promisedSeqNum = -1;
int acceptedSeqNum;
//std::map<std::string, std::map<std::string, std::string>> acceptedVals; // row -> string -> acceptedVal
...
prepare_tuple prepare(int seqNum, std::string row, std::string col) {
	...
	int acceptedSeq = -1;
	if (acceptedVals.count(row) > 0) {
		if (acceptedVals[row].count(col) > 0) {
			acceptedVal = std::get<0>(acceptedVals[row][col]);
			acceptedSeq = std::get<1>(acceptedVals[row][col]);
			retStatus = 1;
		}
	}
	debugDetailed("PREPARE: acceptedVal: %s, acceptedSeqNum: %d\n", acceptedVal.c_str(), acceptedSeq);

	if (seqNum == promisedSeqNum) {
		debugDetailed("PREPARE: %s\n", "seqNum same as promisedSeqNum");
		return std::make_tuple(retStatus, acceptedSeq, acceptedVal);
	} else if (seqNum > promisedSeqNum) {
		debugDetailed("PREPARE: %s\n", "seqNum > promisedSeqNum");
...
// helper to compare responses - returns resp tuple with status code 1 if value was accepted else -1
// note element 1 of prepare_tuple will not be filled correctly
prepare_tuple combineResps(std::map<std::string, prepare_tuple> respSet, std::string val) {
	debugDetailed("---combineReps: %s\n", "entered");
	int numAcks = 0;
	int highestSeqNum = 0;
	std::string chosenVal = val;	
	// check if enough ACKs and chosen value should be
	for (const auto& x : respSet) {
		//x.first is ip:port, x.second is prepare_tuple
		// debugDetailed("---combineReps: %s\n", "respset iter");
    	if (std::get<0>(x.second) == 0) {
    		numAcks = numAcks + 1;
    	} else if (std::get<0>(x.second) == 1) {
    		numAcks = numAcks + 1;
    		int acceptedSeqNum = std::get<1>(x.second);
    		std::string acceptedVal = std::get<2>(x.second);
    		if (acceptedSeqNum >= highestSeqNum) {
    			highestSeqNum = acceptedSeqNum;
    			chosenVal = acceptedVal;
    		}
    	}
	}
	// check if enough acks recieved
	debugDetailed("---combineReps: quorumSize: %d, numAcks: %d, chosenVal: %s\n", quorumSize, numAcks, chosenVal.c_str());
	if (numAcks >= quorumSize) {
		debugDetailed("---combineReps: %s\n", "prepare phase 1 success");
		return std::make_tuple(1, -1, chosenVal);
	} else {
		debugDetailed("---combineReps: %s\n", "prepare phase 1 failure");
		return std::make_tuple(-1, -1, chosenVal);
	}
	// debugDetailed("---combineReps: %s\n", "completed loop over respSet");
	debugDetailed("---combineReps: %s\n", "returned");
...
resp_tuple sendPrepare(std::string row, std::string col, std::string val) {
	std::map<std::string, prepare_tuple> respSet; // map from ip:port -> resp tuple
	prepare_tuple resp;
	bool tryAgain = true;
	...
	while (tryAgain == true) {
		updateSeqNum();
		//resp = prepare(seqNum, row, col);
		respSet[myAddrPortForFrontend] = prepare(seqNum, row, col);
		// loop over other clusters and send prepare
	    for (std::string otherServer : clusterMembers) {
	        //if(clusterNodesToSkip.count(otherServer) <= 0) {
	        if(otherServer.compare(myAddrPortForFrontend) != 0 && clusterNodesToSkip.count(otherServer) <= 0) {
	            bool continueTrying = checkIfNodeIsAlive(otherServer);
	            if(!continueTrying) {
	                debugDetailed("Node %s is dead! Adding node to set of dead nodes.\n", otherServer.c_str());
	            ...
	            while(continueTrying) {
	                ...
	                try { // on timeout, query connection state and retry if connected
	                    ...
	                    debugDetailed("%s for (%s, %s) with server %s timed out!\n", "prepare", row.c_str(), col.c_str(), otherServer.c_str());
	                    // connect to heartbeat thread of backend server and check if it's alive w/ shorter timeout
	                    bool nodeIsAlive = checkIfNodeIsAlive(otherServer);
	                    if(nodeIsAlive) {
	                        // Double timeout and try again if node is still alive
	    ...
	    // choose value from respSet
	    resp = combineResps(respSet, val);
	    ...
	    // loop over other clusters and send accept
	    for (std::string otherServer : clusterMembers) {
	        if(clusterNodesToSkip.count(otherServer) <= 0) {
	        //if(otherServer.compare(myAddrPortForFrontend) != 0 && clusterNodesToSkip.count(otherServer) <= 0) {
	            bool continueTrying = checkIfNodeIsAlive(otherServer);
	            ...
	            while(continueTrying) {
	                ...
	                try { // on timeout, query connection state and retry if connected
	                    ...
	                    debugDetailed("%s for (%s, %s) with server %s timed out!\n", "accept", row.c_str(), col.c_str(), otherServer.c_str());
	                    // connect to heartbeat thread of backend server and check if it's alive w/ shorter timeout
	                    bool nodeIsAlive = checkIfNodeIsAlive(otherServer);
	                    if(nodeIsAlive) {
	                        // Double timeout and try again if node is still alive
...
// Logic: 1) add timeout in primary and nonprimary case for calling put or put approved
        ...
        if (isPrimary()) {
            ...
            while(continueTrying) {
                ...
                try {
                    ...
                    resp = clusterLeaderClient.call("sendPrepare", row, col, val).as<resp_tuple>();
                    printKvLoc();
                    return resp;
                } catch (rpc::timeout &t) {
...
std::tuple<int, std::string> get(std::string row, std::string col) {
	...
	std::string colFile("checkpoint/");
	colFile = colFile + row + "/" + col;

	debugDetailed("---GET entered - row: %s, column: %s, file to read: %s\n", row.c_str(), col.c_str(), colFile.c_str());
	if (lockRow(row) < 0) {
		return std::make_tuple(1, "ERR");
	}
...
void *kvServerThreadFunc(void *arg) {
	...
	srv.bind("decide", decide);
	srv.bind("accept", acceptProposal);
...
int main(int argc, char *argv[]) {	
	...
	sprintf(serverDir, "paxos_%d", serverIdx);
